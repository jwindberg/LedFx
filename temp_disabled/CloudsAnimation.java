...
public class CloudsAnimation implements LedAnimation {
    ...
    private void mapCloudsToLedGrid() {
        // Map to left grid using window coordinates
        for (int y = 0; y < dualLedGrid.getGridSize(); y++) {
            for (int x = 0; x < dualLedGrid.getGridSize(); x++) {
                // Calculate window coordinates for left grid
                ...
                dualLedGrid.setLeftLedColor(x, y, ledColor);
            }
        }
        
        // Map to right grid using window coordinates
        for (int y = 0; y < dualLedGrid.getGridSize(); y++) {
            for (int x = 0; x < dualLedGrid.getGridSize(); x++) {
                // Calculate window coordinates for right grid
                ...
                // Get color from cloud image
                Color ledColor = new Color(cloudImage.getRGB(cloudX, cloudY));
                dualLedGrid.setRightLedColor(x, y, ledColor);
            }
        }
    }
